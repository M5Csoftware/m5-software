...
function CreditLimitReport({
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get(
          `${server}/credit-limit-temp/customer?customerCode=${encodeURIComponent(
            customerCode
          )}`
        );
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const ws = wb.addWorksheet("Credit Limit Report", {
      ...
      const fileName = `Credit_Limit_Report_${customerCode || "All"}_${
    ...
    <>
      <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-start overflow-auto p-2">
        <div className="bg-white w-full h-full p-20 rounded-lg relative flex flex-col">
          ...
          <h2 className="text-2xl font-bold mb-6">Credit Limit Report</h2>
              ...
              <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
                <h2 className="text-sm text-[#000] tracking-wider mr-2 bg-[#D0D5DD] py-[1px] px-2 rounded">
                  {!customerCode
                    ...
                    : rowData.length === 0
                    ? "No Credit Limit Record found for this Customer"
                    : ""}
                </h2>
              </div>
              <TableWithSorting
              ...
              <div className="flex justify-end border-[#D0D5DD] border-opacity-75 border-[1px] border-t-0 text-gray-900 bg-[#D0D5DDB8] rounded rounded-t-none font-sans px-4 py-2 gap-16">
                <div>
                  ...
                  <span className="text-red">
                    ₹ {Number(leftOverBalance || 0).toFixed(2)}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex gap-16">
                    ...
                    <div>
                      ...
                      <span className="text-red">
                        {rowData
                          .reduce((s, x) => s + (Number(x.debitAmount) || 0), 0)
                          .toFixed(2)}
                      </span>
                    </div>
