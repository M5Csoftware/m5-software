  ...
  const fetchShipmentData = async (awb) => {
    try {
      ...
      if (data) {
        ...
        setBoxCount(boxCount > 1 ? boxCount - 1 : 0);

        let generatedRows = [];

        if (boxCount === 1) {
          // Single box: no children to create
          showNotification("info", "Single piece shipment - no child shipments needed");
          generatedRows = [];
        } else {
          // Multiple boxes: create children for boxes 2 onwards (skip first box which is master)
          if (!isNaN(childSrNo) && !isNaN(jump)) {
            // Start from index 1 to skip the first box (master)
          ...
          <div>
            ...
            {/* <div>
              <SimpleButton
                ...
                type="submit"
                onClick={handleSave}
                disabled={hasExistingChildren || checkingExistence}
              />
            </div> */}
          </div>
          <div className="">
