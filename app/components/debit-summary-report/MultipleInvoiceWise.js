...
const MultipleInvoiceWise = ({
  ...
  const fetchDebitNotesWithPagination = async (filters, page = 1) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${server}/debit-summary-report/multiple-invoice`,
        payload
      );
      ...
      showNotification("error", "Failed to fetch debit summary data");
      setRowData([]);
      setTotalRecords(0);
      setTotalPages(1);
    } finally {
  ...
  const handleShow = async () => {
    ...
    // Fetch first page
    await fetchDebitNotesWithPagination(filters, 1);
  };
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Debit Multiple Invoice");

      // Generate filename with current date
      const fileName = `Debit_Multiple_Invoice_Report_${
          ...
          <div className="fixed inset-0 z-50 bg-white flex flex-col p-12">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">
                Debit Summary Report Multiple Invoice Wise
              </h2>
              <button
