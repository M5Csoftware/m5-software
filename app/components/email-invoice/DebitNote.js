...
function DebitNote() {
    ...
    try {
      ...
      if (response.data.success && response.data.debitNotes) {
        ...
        const formattedData = await Promise.all(
          debitNotes.map(async (debitNote) => {
            let email = "";
        ...
        if (formattedData.length > 0) {
          setNotification({
            type: "success",
            message: `Found ${formattedData.length} debit note(s)`,
            visible: true,
          });
        } else {
          setNotification({
            type: "info",
            message: "No debit notes found matching the criteria",
            visible: true,
          });
        }
      } else {
        ...
        setNotification({
          type: "info",
          message: response.data.message || "No debit notes available",
          visible: true,
        });
      }
    } catch (error) {
      ...
      setNotification({
        type: "error",
        message: error.response?.data?.message || "Failed to fetch debit notes",
        visible: true,
      });
      setRowData([]);
    } finally {
  ...
  const handleGeneratePDF = async () => {
    if (selectedItems.length === 0) {
      setNotification({
        type: "error",
        message: "Please select at least one debit note",
        visible: true,
      });
      return;
    }
    ...
    if (itemsWithoutEmail.length > 0) {
      setNotification({
        type: "error",
        message: `${itemsWithoutEmail.length} debit note(s) missing customer email. Cannot generate PDF.`,
        visible: true,
      });
      return;
    }
    ...
    setNotification({
      type: "info",
      message: `Preparing ${selectedItems.length} debit note(s) for PDF generation...`,
      visible: true,
    });
    ...
    setTimeout(() => {
      ...
      setNotification({
        type: "success",
        message: `${selectedItems.length} debit note(s) ready to send. Click 'Send Email' to proceed.`,
        visible: true,
      });
    }, 500);
  };
  ...
  const handleSendEmail = async () => {
    ...
    if (itemsWithoutEmail.length > 0) {
      setNotification({
        type: "error",
        message: `${itemsWithoutEmail.length} debit note(s) missing customer email`,
        visible: true,
      });
      return;
    }
    ...
    try {
      const response = await axios.post(`${server}/email-debit-note`, {
        selectedDebitNotes: selectedItems,
      });
