...
function CreditNote() {
  ...
  const toISODate = (val) => {
    ...
    if (/^\d{2}\/\d{2}\/\d{4}$/.test(val)) {
      ...
      // force DD/MM/YYYY (India standard in your app)
      ...
      if (response.data.success && response.data.creditNotes) {
        ...
        const formattedData = await Promise.all(
          creditNotes.map(async (creditNote) => {
            ...
            if (creditNote.customerCode) {
              try {
                const customerResponse = await axios.get(
                  `${server}/customer-account?accountCode=${creditNote.customerCode}`
                );
                ...
                console.error(
                  `Error fetching email for ${creditNote.customerCode}:`,
                  error
                );
              }
            }
            ...
            return {
              ...
              grandTotal: `₹${creditNote.grandTotal.toFixed(2)}`,
            };
          })
        );
        ...
        if (formattedData.length > 0) {
          setNotification({
            type: "success",
            message: `Found ${formattedData.length} credit note(s)`,
            visible: true,
          });
        } else {
          setNotification({
            type: "info",
            message: "No credit notes found matching the criteria",
            visible: true,
          });
        }
      } else {
        ...
        setNotification({
          type: "info",
          message: response.data.message || "No credit notes available",
          visible: true,
        });
      }
    } catch (error) {
  ...
  const handleGeneratePDF = async () => {
    ...
    // Validate that all selected credit notes have emails
    ...
    if (itemsWithoutEmail.length > 0) {
      setNotification({
        type: "error",
        message: `${itemsWithoutEmail.length} credit note(s) missing customer email. Cannot generate PDF.`,
        visible: true,
      });
      return;
    }
    ...
    setNotification({
      type: "info",
      message: `Preparing ${selectedItems.length} credit note(s) for PDF generation...`,
      visible: true,
    });
    ...
    setTimeout(() => {
      ...
      setNotification({
        type: "success",
        message: `${selectedItems.length} credit note(s) ready to send. Click 'Send Email' to proceed.`,
        visible: true,
      });
    }, 500);
  };
  ...
  const handleSendEmail = async () => {
    ...
    if (itemsWithoutEmail.length > 0) {
      setNotification({
        type: "error",
        message: `${itemsWithoutEmail.length} credit note(s) missing customer email`,
        visible: true,
      });
      return;
    }
    ...
    try {
      const response = await axios.post(`${server}/email-credit-note`, {
        selectedCreditNotes: selectedItems,
      });
