...
function InvoiceDetails({
  ...
  useEffect(() => {
    ...
    console.log("Validation check:", Object.keys(rowData).length, newValidationStatus);
    setValidationStatus(newValidationStatus);

  }, [rowData, globalTotalPcs]);

  const fetchAwbData = async (awbNumber) => {
    if (!awbNumber || hasFetched) return; // prevent refetch if already done
    try {
  ...
  const populateInvoiceData = (shipmentAndPackageDetails) => {
    ...
    Object.keys(shipmentAndPackageDetails).forEach((boxNumber) => {
      ...
      newRowData[boxNum] = shipmentAndPackageDetails[boxNumber].map(
        (item, index) => ({
          id: generateId(),
  ...
  const findHsnCode = (content) => {
    ...
    return matchedProduct ? matchedProduct.hsnCode : "";
  };

  // Watch for content changes to auto-fill HSN code
  useEffect(() => {
    const content = watchedValues.context;
    if (content && content.trim() !== "") {
      const hsnCode = findHsnCode(content);
      if (hsnCode && hsnCode !== watchedValues.hsnNo) {
        setValue("hsnNo", hsnCode);
      }
    }
  }, [watchedValues.context, watchedValues.hsnNo, setValue]);
  ...
  return (
    <>
      <div
        className={`flex flex-col gap-4 bg-white px-9 py-8  fixed inset-x-48 inset-y-10 z-40 border rounded-md ${window ? "" : "hidden"
          }`}
      >
          ...
          <div className="flex gap-2">
            <div className="w-[31vw]">
              <NumberInputBox
                ...
                validation={{
                  required: "boxNumber is required",
                }}
              />
            </div>
            ...
            <InputBox
              ...
              setValue={setValue}
              value="context"
              initialValue={watchedValues?.context || ""}
              resetFactor={resetForm}
              validation={{
        ...
        <div className="flex flex-col justify-between h-full">
          ...
          <div className="flex justify-between bottom-0">
            ...
            <div className="flex gap-4 items-center">
              <div
                className={`py-2 px-4 rounded-md flex gap-5 text-center items-center ${!validationStatus.pcsMatch || !validationStatus.boxMatch
        ...
        {Object.keys(rowData).length > 0 && (
          <div
            className={`flex items-center justify-between px-4 py-2 rounded-md text-sm font-medium shadow-sm transition-all duration-300 ${validationStatus
          ...
          >
            <div className="flex items-center gap-2">
              <span
                className={`flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold ${validationStatus ? "bg-green-500 text-white" : "bg-white text-red"
                  }`}
              >
                {validationStatus ? "✓" : "!"}
              </span>
...
function TableRow({
  ...
  return (
    <>
      <tr
        className={`border-b border-alice-blue h-11 ${isEditing ? "bg-blue-50" : ""
          }`}
      >
