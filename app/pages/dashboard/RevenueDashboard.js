    ...
    const fetchMonthlyData = async () => {
      try {
        const response = await axios.get(
          `${server}/dashboard/revenue-dashboard-data`,
        );
        ...
        if (response.status === 200) {
          ...
          setMonthlyStateData(
            data.monthlyStateData || {
              [currentYearMonth]: [],
            },
          );

          setMonthlySectorData(
            data.monthlySectorData || {
              [currentYearMonth]: [],
            },
          );
          setHubWiseData(
  ...
  return (
    <div className="flex gap-7 flex-grow w-full">
      ...
      <div className="flex flex-col gap-6 flex-grow justify-between">
        <DashboardTopListCard city="New Delhi" title="Top Sales Person" />
        <DashboardTopListCard city="New Delhi" title="Top Customers" />
      </div>
    </div>
  );
}
