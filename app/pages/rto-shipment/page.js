...
function RTOShipment() {
  ...
  const confirmDelete = async () => {
    if (deleteIndex !== null) {
      const deletedRow = rowData[deleteIndex];
      ...
      // Log AWB deletion
      const customerName = await getCustomerName(deletedRow.accountCode);
      await pushAWBLog({
        awbNo: deletedRow.awbNo,
        accountCode: deletedRow.accountCode,
        customer: customerName,
        action: "Removed from RTO Shipment",
        actionUser: user?.userId,
        department: "Operations",
      });
  ...
  const handleSendAlert = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        // Log AWB alert sent for each shipment
        for (const shipment of rowData) {
          const customerName = await getCustomerName(shipment.accountCode);
          await pushAWBLog({
            awbNo: shipment.awbNo,
            accountCode: shipment.accountCode,
            customer: customerName,
            action: "RTO Alert Sent",
            actionUser: user?.userId,
            department: "Operations",
          });
        }
